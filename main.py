...
EQ_FREQS_HZ = [32, 64, 125, 250, 500, 1000, 2000, 4000, 8000, 16000]
EQ_PRESETS = {
    ...
    "Classical": [4, 4, 3, 2, -1, -2, -2, 0, 2, 4],
    "Hip-Hop": [5, 4, 2, 1, -1, -1, 0, 2, 3, 4],
    "Electronic": [5, 4, 2, 0, -2, -2, 0, 3, 5, 5],
    "Vocal": [-2, -1, 0, 3, 5, 5, 3, 0, -1, -2],
}


def find_equalizer_apo_config():
    """Find Equalizer APO config file path"""
    possible_paths = [
        os.path.join(os.environ.get("ProgramFiles", "C:\\Program Files"), "EqualizerAPO", "config", "config.txt"),
        os.path.join(os.environ.get("ProgramFiles(x86)", "C:\\Program Files (x86)"), "EqualizerAPO", "config", "config.txt"),
...
def generate_apo_config(eq_values, enabled=True):
    """Generate Equalizer APO config from EQ values"""
    lines = ["# Generated by Malware Confinement", ""]
    
    if not enabled:
        lines.append("# Equalizer disabled")
        return "\n".join(lines)
    
    lines.append("Preamp: -6 dB")
    lines.append("")
    
    for freq_hz, gain in zip(EQ_FREQS_HZ, eq_values):
        if gain != 0:
            lines.append(f"Filter: ON PK Fc {freq_hz} Hz Gain {gain} dB Q 1.0")
...
class EqualizerDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setModal(False)
        self.sliders = []
        self.eq_file = os.path.join(DATA_DIR, "eq_config.txt")
        self.apo_config_path = find_equalizer_apo_config()
        self.setup_ui()
        self.load_eq_state()
    ...
    def setup_ui(self):
        ...
        if self.apo_config_path:
            status_label = QLabel(f"✓ Equalizer APO detected")
            status_label.setStyleSheet("color: #00b894; font-size: 11px; font-weight: bold;")
        else:
            status_label = QLabel("⚠ Equalizer APO not found")
            status_label.setStyleSheet("color: #f39c12; font-size: 11px; font-weight: bold;")
        status_layout.addWidget(status_label)
        status_layout.addStretch()
        
        if not self.apo_config_path:
            install_btn = QPushButton("Download APO")
            install_btn.setFixedWidth(100)
            install_btn.clicked.connect(self.open_apo_download)
        ...
        self.toggle_btn.clicked.connect(self.toggle_eq)
        self.toggle_btn.setStyleSheet("background-color: #00b894; color: #0d0d1a; border-radius: 6px; font-weight: bold;")
        preset_layout.addWidget(self.toggle_btn)
        ...
        for i, freq in enumerate(EQ_FREQS):
            ...
            label.setStyleSheet("font-size: 11px; color: #8888aa;")
            ...
            slider.valueChanged.connect(self.on_eq_change)
            self.sliders.append(slider)
            self.grid_layout.addWidget(slider, 1, i)

            val_label = QLabel("0 dB")
            val_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            val_label.setStyleSheet("font-size: 10px; color: #8888aa;")
            self.value_labels.append(val_label)
            self.grid_layout.addWidget(val_label, 2, i)
        ...
        if self.apo_config_path:
            info_label = QLabel("Changes apply to Equalizer APO in real-time")
            info_label.setStyleSheet("font-size: 10px; color: #00b894;")
        else:
            info_label = QLabel("Install Equalizer APO for real audio processing")
            info_label.setStyleSheet("font-size: 10px; color: #555570;")
    ...
    def on_eq_change(self):
        for i, slider in enumerate(self.sliders):
            val = slider.value()
            self.value_labels[i].setText(f"{val} dB" if val != 0 else "0 dB")
        self.save_eq_state()
        self.apply_to_apo()
    ...
    def toggle_eq(self):
        ...
        if self.eq_enabled:
            ...
            self.toggle_btn.setStyleSheet("background-color: #e74c3c; color: #0d0d1a; border-radius: 6px; font-weight: bold;")
        self.save_eq_state()
        self.apply_to_apo()

    def apply_to_apo(self):
        """Apply EQ settings to Equalizer APO config"""
        if not self.apo_config_path:
            return
        
        try:
            values = [s.value() for s in self.sliders]
            config_content = generate_apo_config(values, self.eq_enabled)
            
            # Create backup
            backup_path = self.apo_config_path + ".backup"
            if os.path.exists(self.apo_config_path) and not os.path.exists(backup_path):
                import shutil
                shutil.copy2(self.apo_config_path, backup_path)
            
            # Write new config
            with open(self.apo_config_path, "w", encoding="utf-8") as f:
                f.write(config_content)
        except Exception as e:
            print(f"Error writing APO config: {e}")

    def open_apo_download(self):
        """Open Equalizer APO download page"""
        import webbrowser
        webbrowser.open("https://sourceforge.net/projects/equalizerapo/")

    def save_eq_state(self):
        values = [s.value() for s in self.sliders]
        with open(self.eq_file, "w") as f:
            f.write(f"enabled: {self.eq_enabled}\n")
            for freq, val in zip(EQ_FREQS, values):
                f.write(f"{freq}: {val}\n")

    def load_eq_state(self):
        if os.path.exists(self.eq_file):
            try:
                with open(self.eq_file, "r") as f:
                    for line in f:
                        line = line.strip()
                        if line.startswith("enabled:"):
                            self.eq_enabled = line.split(":")[1].strip() == "True"
                            if self.eq_enabled:
                                self.toggle_btn.setText("ON")
                                self.toggle_btn.setStyleSheet("background-color: #00b894; color: #0d0d1a; border-radius: 6px; font-weight: bold;")
                            else:
...
class MusicPlayer(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # RIGHT: volume + audio device (fixed 280px)
        ...
        self.audio_device_combo.setFixedHeight(28)
        ...
        """)
        self.audio_device_combo.currentIndexChanged.connect(self.switch_audio_device)
        device_row.addWidget(self.audio_device_combo)

        self.sound_settings_btn = QPushButton("⚙")
        self.sound_settings_btn.setObjectName("small_btn")
        self.sound_settings_btn.setFixedWidth(28)
        self.sound_settings_btn.setFixedHeight(28)
        self.sound_settings_btn.setToolTip("Sound settings")
        self.sound_settings_btn.clicked.connect(self.open_sound_settings)
        device_row.addWidget(self.sound_settings_btn)

        self.eq_btn = QPushButton("🎛")
        self.eq_btn.setObjectName("small_btn")
        self.eq_btn.setFixedWidth(28)
        self.eq_btn.setFixedHeight(28)
        ...
        volume_row.addWidget(self.volume_btn)
    ...
    def refresh_audio_devices(self):
        ...
        for i, dev in enumerate(devices):
            self.audio_device_combo.addItem(dev.description(), dev)
            if dev == default:
                default_idx = i
        self.audio_device_combo.setCurrentIndex(default_idx)
    ...
    def switch_audio_device(self):
        ...
        # Small delay to ensure clean device switch
        QApplication.processEvents()
        
        # Create new audio output
        try:
            new_output = QAudioOutput(dev)
            new_output.setVolume(vol)
            new_output.setMuted(muted)
            
            # Delete old audio output to free resources
            ...
            self.statusBar().showMessage(f"Switched to: {dev.description()} (Volume: {int(vol*100)}%)", 2000)
        except Exception as e:
    ...
    def open_equalizer(self):
        if self.eq_dialog is None or not self.eq_dialog.isVisible():
            self.eq_dialog = EqualizerDialog(self)
        self.eq_dialog.show()
